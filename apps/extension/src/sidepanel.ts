...
const interim = document.querySelector<HTMLElement>("#interim")!;
...
const finalized: Array<{ id: string; time: string; text: string; confidence: number }> = [];
let scoreController: AbortController | undefined;
function send(message: SessionCommand) { chrome.runtime.sendMessage(message).catch(() => undefined); }
...
async function scoreTranscript() {
  ...
  const payload = await response.json() as { snapshot?: { smoothed?: { overall?: number } } };
  score.textContent = `Local score: ${payload.snapshot?.smoothed?.overall ?? "--"}`;
}

chrome.runtime.onMessage.addListener((message: SttMessage) => {
  if (message.type === "session/status") {
    ...
    void scoreTranscript().catch((error: Error) => { if (error.name !== "AbortError") score.textContent = "Local scorer offline"; });
...
document.querySelector<HTMLButtonElement>("#stop")!.addEventListener("click", () => send({ type: "session/stop" }));
